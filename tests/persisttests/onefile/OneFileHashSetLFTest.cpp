...
int main(int argc, char **argv) {
    ...
    oflf::OneFileLF::updateTx<bool>([&]() {
        set = oflf::OneFileLF::tmNew<OFLFResizableHashSet<uint64_t>>(thread_number, 1000);
    });
    Tracer tracer;
    tracer.startTime();
    simpleInsert();
    ...
    oflf::OneFileLF::updateTx<bool>([&]() { oflf::OneFileLF::tmDelete(set); });
    delete[] output;
}