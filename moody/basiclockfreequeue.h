...
private:
    ...
    inline idx_t try_get_node_from_free_list() {
        ...
        while (head != 0 &&
               !freeListHead.compare_exchange_weak(head, get_node_at(head)->next.load(std::memory_order_relaxed),
    ...
    inline Node *get_node_at(idx_t idx) {
        auto uberBlock = uberBlockContainer.subblocks[(idx & UBER_BLOCK_IDX_MASK) >> UBER_BLOCK_IDX_SHIFT].load(
                std::memory_order_relaxed);
        auto ultraBlock = uberBlock->subblocks[(idx & UBER_BLOCK_MASK) >> UBER_BLOCK_SHIFT].load(
                std::memory_order_relaxed);
        auto superBlock = ultraBlock->subblocks[(idx & ULTRA_BLOCK_MASK) >> ULTRA_BLOCK_SHIFT].load(
                std::memory_order_relaxed);
        auto block = superBlock->subblocks[(idx & SUPER_BLOCK_MASK) >> SUPER_BLOCK_SHIFT].load(
                std::memory_order_relaxed);
    ...
    inline TSubBlock *lookup_subblock(TBlock *block, std::size_t idx) {
        ...
        if (ptr == nullptr) {
            ...
            if (!block->subblocks[idx].compare_exchange_strong(ptr, newBlock, std::memory_order_release,
                                                               std::memory_order_acquire)) {
                delete newBlock;
            } else {
                ptr = newBlock;
...
public:
    BasicLockFreeQueue() : nextNodeIdx(2), freeListHead(0) {
    ...
    inline bool try_dequeue(T &item) {
        while (true) {
            ...
            if (next == 0) {
                ...
                if (head.compare_exchange_strong(head_, set_consumed_flag(head_), std::memory_order_release,
                                                 std::memory_order_relaxed)) {
