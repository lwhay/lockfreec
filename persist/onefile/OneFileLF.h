...
#ifndef LOCKFREEC_ONEFILELF_H
#define LOCKFREEC_ONEFILELF_H
...
// Maximum number of stores in the WriteSet per transaction
    static const uint64_t TX_MAX_STORES = 5 * 1024 * 1024;
// Number of buckets in the hashmap of the WriteSet.
    static const uint64_t HASH_BUCKETS = 1024 * 1024;
    ...
    public:
        HazardErasOF() {
            ...
            for (unsigned it = 0; it < REGISTRY_MAX_THREADS; it++) {
                ...
                retiredList[it * CLPAD].reserve(
                        REGISTRY_MAX_THREADS);  // We pre-reserve one object per thread, should be enough to start
            }
        }
...
// Its purpose is to hold thread-local data
    struct OpData {
        uint64_t curTx{
                0};                   // Used during a transaction to keep the value of currTx read in beginTx() (owner thread only)
...
//
    ...
    template<typename T, typename... Args>
    T *tmNew(Args &&... args) { return OneFileLF::tmNew<T>(args...); }
