...
#ifndef LOCKFREEC_ROMULUSLR_H
#define LOCKFREEC_ROMULUSLR_H
                ...
                for (size_t idx = 0; idx < g_main_size - sizeof(size_t); idx++) {
                    if (*(main_addr + idx) != *(back_addr + idx)) {
                        printf("Difference at %p  main=%ld  back=%ld\n", main_addr + idx,
                               *(int64_t *) (main_addr + idx), *(int64_t *) (back_addr + idx));
                        sumdiff++;
        ...
        void ns_write_transaction(Func &&mutativeFunc) {
            ...
            while (true) {
                ...
                if (writersMutex.load() == UNLOCKED &&
                    writersMutex.compare_exchange_strong(unlocked, LOCKED))
                    break;
            ...
            if (tl_nested_write_trans > 0) {
                ...
                while (true) {
                    ...
                    if (gRomLR.writersMutex.load() == UNLOCKED &&
                        gRomLR.writersMutex.compare_exchange_strong(unlocked, LOCKED))
                        break;
                    std::this_thread::yield();
                }
    ...
    struct persist {
        ...
        T *operator&() {
            ...
            if (valaddr > g_main_addr && valaddr < g_main_addr_end)
                return reinterpret_cast<T *>((uint8_t *) &val + g_main_size );
            return &val;
        }
