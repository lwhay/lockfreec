...
#ifdef __APPLE__
    static const char *PFILE_NAME = "./ponefilelf_shared";
#else
 ...
 */
    ...
    public:
        void init(void *addressOfMemoryPool, size_t sizeOfMemoryPool, bool clearPool = true) {
            ...
            if (debugOn)
                printf("Starting EsLoco with poolAddr=%p and poolSize=%ld, up to %p\n", poolAddr, poolSize,
                       poolAddr + poolSize);
        }
...
// Its purpose is to hold thread-local data
    struct OpData {
        uint64_t curTx{
                0};              // Used during a transaction to keep the value of curTx read in beginTx() (owner thread only)
