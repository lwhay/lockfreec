...
#ifndef LOCKFREEC_BTREES_H
#define LOCKFREEC_BTREES_H
...
BtDb *bt_open(char *name, uint mode, uint bits, uint nodemax, uint pgblk) {
    ...
    switch (mode & 0x7fff) {
        ...
        case BT_rw:
            bt->idx = CreateFile(name, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                                 OPEN_ALWAYS, attr, NULL);
            break;

        case BT_ro:
        default:
            bt->idx = CreateFile(name, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, attr,
                                 NULL);
            lockmode = BtLockRead;
            break;
    }
