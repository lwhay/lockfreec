...
BTERR
bt_insertslot(BtDb *bt, BtPageSet *set, uint slot, unsigned char *key, uint keylen, unsigned char *value, uint vallen,
...
BTERR bt_atomicinsert(BtDb *bt, BtPage source, AtomicTxn *locks, uint src) {
    ...
    while (slot = bt_atomicpage(bt, source, locks, src, set)) {
        if (slot = bt_cleanpage(bt, set, key->len, slot, val->len))
            return bt_insertslot(bt, set, slot, key->key, key->len, val->value, val->len, slotptr(source, src)->type,
                                 0);
...
int bt_atomictxn(BtDb *bt, BtPage source) {
    ...
    for (src = 0; src++ < source->cnt;) {
        ...
        if (samepage = src > 1)
            if (samepage = !bt_getid(set->page->right) ||
                           keycmp(keyptr(set->page, set->page->cnt), key->key, key->len) >= 0)
    ...
    *(ushort *) (bt->mgr->lock) = 0;
    ...
    for (hashidx = 0; hashidx < bt->mgr->latchhash; hashidx++) {
        ...
        *(ushort *) (bt->mgr->hashtable[hashidx].latch) = 0;
        ...
        if (idx = bt->mgr->hashtable[hashidx].slot)
            do {
                latch = bt->mgr->latchsets + idx;
                if (latch->pin)
                    fprintf(stderr, "latchset %d pinned for page %.8x\n", idx, latch->page_no);
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        fprintf(stderr,
                "  cmds is a string of (c)ount/(r)ev scan/(w)rite/(s)can/(d)elete/(f)ind/(p)ennysort, with one character"
                " command for each input src_file. Commands with no input file need a placeholder.\n");
