...
logseqno bt_newredo(BtMgr *mgr, BTRM type, int lvl, BtKey *key, BtVal *val, ushort thread_no) {
    ...
    if (amt > size - mgr->redoend) {
        ...
        if (msync(mgr->redobuff + (mgr->redolast & ~0xfff), mgr->redoend - (mgr->redolast & ~0xfff) + sizeof(BtLogHdr),
                  MS_SYNC) < 0)
...
BtLatchSet *bt_pinlatch(BtMgr *mgr, uid page_no, ushort thread_no) {
    ...
    if (entry = mgr->hashtable[hashidx].entry)
        do {
            latch = mgr->latchsets + entry;
...
BtMgr *bt_mgr(char *name, uint pagebits, uint leafxtra, uint nodemax, uint leafmax, uint redopages) {
    ...
    mgr->idx = CreateFile(name, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS,
                          attr, NULL);
    ...
    *amt = 0;
    ...
    mgr->latchhash = (mgr->pagepool + ((uid) mgr->nlatchpage << mgr->page_bits) - (unsigned char *) mgr->hashtable) /
                     sizeof(BtHashEntry);
    ...
    mgr->leafhash = (mgr->leafpool + ((uid) mgr->nleafpage << mgr->page_bits) - (unsigned char *) mgr->leaftable) /
                    sizeof(BtHashEntry);
...
BTERR
bt_insertslot(BtMgr *mgr, BtPageSet *set, uint slot, unsigned char *key, uint keylen, unsigned char *value, uint vallen,
              uint type) {
    uint idx, librarian;
    BtSlot *node;
    BtKey *ptr;
...
BTERR bt_atomicinsert(BtMgr *mgr, BtPage source, AtomicTxn *locks, uint idx, ushort thread_no, logseqno lsn) {
    ...
    while (slot = bt_atomicpage(mgr, source, locks, idx, set)) {
        if (slot = bt_cleanpage(mgr, set, key->len, slot, val->len)) {
            if (bt_insertslot(mgr, set, slot, key->key, key->len, val->value, val->len,
                              slotptr(source, locks[idx].src)->type))
                return mgr->err_thread = thread_no, mgr->err;
...
BTERR bt_promote(BtDb *bt) {
    ...
    while (1) {
        ...
        if (bt_atomicexec(bt->main, set->page, set->page->cnt, 0, bt->mgr->pagezero->redopages ? 1 : 0,
                          bt->thread_no)) {
...
#endif
{
    ...
    switch (ch | 0x20) {
        case 'm':
            ...
            if (bt->main)
                if (bt_flushmain(bt))
                    fprintf(stderr, "Error %d Line: %d thread: %d\n", bt->mgr->err, bt->mgr->line, bt->thread_no), exit(
                            0);
                ...
                while (ch = getc(in), ch != EOF)
                    if (ch == '\n') {
                        ...
                        if (bt_insertkey(bt->mgr, key, len, 0, NULL, 0, Unique, bt->thread_no))
                            fprintf(stderr, "Error %d Line: %d source: %d\n", bt->mgr->err, bt->mgr->line, line), exit(
                                    0);
                        len = 0;
                    } else if (len < BT_maxkey)
                        key[len++] = ch;
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr,
                "Usage: %s idx_file main_file cmds [pagebits leafbits poolsize leafpool txnsize redopages mainbits mainleafbits mainpool mainleafpool src_file1 src_file2 ... ]\n",
                argv[0]);
        ...
        fprintf(stderr,
                "  cmds is a string of (r)ev scan/(w)rite/(s)can/(d)elete/(f)ind/(p)ennysort/(c)ount/(m)ainflush, with a"
                " one character command for each input src_file. A command can also be given with no input file\n");
    ...
    fprintf(stderr, "cache %lld leaves %lld upper %d reads %d writes %d found\n", mgr->pagezero->leafpages,
            mgr->pagezero->upperpages, mgr->reads, mgr->writes, mgr->found);
    if (main)
        fprintf(stderr, "main %lld leaves %lld upper %d reads %d writes %d found\n", main->pagezero->leafpages,
                main->pagezero->upperpages, main->reads, main->writes, main->found);
