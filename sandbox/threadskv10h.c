...
extern BTERR
bt_insertkey(BtMgr *mgr, unsigned char *key, uint len, uint lvl, void *value, uint vallen, BtSlotType type);
...
BtPage bt_mappage(BtMgr *mgr, BtLatchSet *latch) {
    ...
    while (1) {
        ...
        if (mgr->segments < mgr->maxseg) {
            mgr->pages[mgr->segments] = mmap(0, (uid) mgr->page_size << SEG_bits, flag, MAP_SHARED, mgr->idx,
                                             (uid) mgr->segments << mgr->page_bits << SEG_bits);
            mgr->segments++;
            continue;
        }
...
BtMgr *bt_mgr(char *name, uint pagebits, uint leafxtra, uint nodemax) {
	...
	*amt = 0;
    ...
    pagezero->latchhash =
            (((uid) pagezero->nlatchpage << mgr->page_bits) - nodemax * sizeof(BtLatchSet)) / sizeof(BtHashEntry);
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr,
                "Usage: %s idx_file main_file cmds [pagebits leafbits poolsize txnsize mainbits mainleafbits mainpool maxleaves src_file1 src_file2 ... ]\n",
                argv[0]);
        ...
        fprintf(stderr,
                " cmds is a string of (r)ev scan/(w)rite/(s)can/(d)elete/(f)ind/(p)ennysort/(c)ount/(m)ainflush/(a)udit,"
                " with a one character command for each input src_file. A command can also be given with no input file\n");
