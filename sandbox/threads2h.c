// btree version threads2h pthread rw lock/SRW version
//	with fixed bt_deletekey code
// 17 FEB 2014
...
BtLatchSet *bt_pinlatch(BtDb *bt, uid page_no) {
    ...
    if (slot = bt->mgr->latchmgr->table[hashidx].slot)
        do {
...
BtMgr *bt_mgr(char *name, uint mode, uint bits, uint poolmax, uint segsize, uint hashsize) {
    ...
    *amt = 0;
    ...
    if (mgr->latchmgr == MAP_FAILED)
        return bt_mgrclose(mgr), NULL;
    mgr->latchsets = (BtLatchSet *) mmap(0, mgr->latchmgr->nlatchpage * mgr->page_size, flag, MAP_SHARED, mgr->idx,
                                         LATCH_page * mgr->page_size);
    if (mgr->latchsets == MAP_FAILED)
        return bt_mgrclose(mgr), NULL;
#else
    flag = PAGE_READWRITE;
    mgr->halloc = CreateFileMapping(mgr->idx, NULL, flag, 0,
                                    (BT_latchtable / (mgr->page_size / sizeof(BtLatchSet)) + 1 + LATCH_page) *
                                    mgr->page_size, NULL);
    if (!mgr->halloc)
        return bt_mgrclose(mgr), NULL;

    flag = FILE_MAP_WRITE;
    mgr->latchmgr = MapViewOfFile(mgr->halloc, flag, 0, 0,
                                  (BT_latchtable / (mgr->page_size / sizeof(BtLatchSet)) + 1 + LATCH_page) *
                                  mgr->page_size);
    if (!mgr->latchmgr)
...
BTERR bt_mapsegment(BtDb *bt, BtPool *pool, uid page_no) {
    ...
    pool->map = MapViewOfFile(pool->hmap, flag, (DWORD) (off >> 32), (DWORD) off,
                              (bt->mgr->poolmask + 1) << bt->mgr->page_bits);
    if (!pool->map)
        return bt->err = BTERR_map;
#endif
    return bt->err = 0;
...
uid bt_newpage(BtDb *bt, BtPage page) {
    ...
    if (!reuse && bt->mgr->poolmask > 0 && (new_page & bt->mgr->poolmask) == 0) {
        ...
        if (pwrite(bt->mgr->idx, bt->zero, bt->mgr->page_size, (new_page | bt->mgr->poolmask) << bt->mgr->page_bits) <
            bt->mgr->page_size)
            return bt->err = BTERR_wrt, 0;
    }
#else
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr,
                "Usage: %s idx_file Read/Write/Scan/Delete/Find [page_bits mapped_segments seg_bits line_numbers src_file1 src_file2 ... ]\n",
                argv[0]);
