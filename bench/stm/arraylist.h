...
public:
    ...
    inline bool isFull() {
        return __size.load(std::memory_order_relaxed) ==
               capacity; // note: this must be seq_cst if membars are not manually added
    }

    inline bool isEmpty() {
        return __size.load(std::memory_order_relaxed) ==
               0; // note: this must be seq_cst if membars are not manually added
    }
};
...
public:
    ...
    ArrayList(const int _capacity) : capacity(_capacity) {
        __size = 0;
        data = (new T *[capacity + 2 * PREFETCH_SIZE_WORDS]) +
               PREFETCH_SIZE_WORDS; /* HACKY OVER-ALLOCATION AND POINTER SHIFT TO ADD PADDING ON EITHER END WITH MINIMAL ARITHEMTIC OPS */
    }
