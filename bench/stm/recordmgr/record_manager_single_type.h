...
public:
    ...
    record_manager_single_type(const int numProcesses, RecoveryMgr<void *> *const _recoveryMgr)
        ...
        VERBOSE
        DEBUG COUTATOMIC("constructor record_manager_single_type" << std::endl);
    ...
    ~record_manager_single_type() {
        VERBOSE
        DEBUG COUTATOMIC("destructor record_manager_single_type" << std::endl);
        delete reclaim;
        delete pool;
        delete alloc;
    }
...
//        endOp(tid);
    ...
    inline bool protect(const int tid, record_pointer obj, CallbackType notRetiredCallback, CallbackArg callbackArg,
    ...
    inline bool qProtect(const int tid, record_pointer obj, CallbackType notRetiredCallback, CallbackArg callbackArg,
                         bool hintMemoryBarrier = true) {
