...
//    PAD;
    ...
    record_manager(const int numProcesses,
                   const int _neutralizeSignal = -1 /* unused except in conjunction with special DEBRA+ memory reclamation */)
            : NUM_PROCESSES(numProcesses),
              recoveryMgr(new RecoveryMgr<SelfType>(numProcesses, _neutralizeSignal, this)) {
        rmset = new RecordManagerSetPostPadded<Reclaim, Alloc, Pool, RecordTypesFirst, RecordTypesRest...>(numProcesses,
...
//        endOp(tid);
    ...
    inline bool isQuiescent(const int tid) {
        return rmset->get((RecordTypesFirst *) NULL)->isQuiescent(
                tid); // warning: if quiescence information is logically shared between all types, with the actual data being associated only with the first type (as it is here), then isQuiescent will return inconsistent results if called in functions that recurse on the template argument list in this class.
    }
...
//        GSTATS_ADD_IX(tid, num_prop_epoch_allocations, 1, GSTATS_GET(tid, thread_announced_epoch));
    ...
    public:
        MemoryReclamationGuard(const int _tid,
                               record_manager<Reclaim, Alloc, Pool, RecordTypesFirst, RecordTypesRest...> *_recmgr,
