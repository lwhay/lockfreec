...
//    PAD; // not needed after superclass layout
    ...
    inline bool neutralizeOther(const int tid, const int otherTid, const long currentEpoch, const long announceOther) {
        ...
        if (epochbags[NUMBER_OF_EPOCH_BAGS_CR * tid + index[tid * PREFETCH_SIZE_WORDS]]->getSizeInBlocks() >=
            NEUTRALIZE_THRESHOLD_IN_BLOCKS) {
...
//                COUTATOMICTID("sending signal to tid "<<otherTid<<std::endl);
            ...
            if (error = pthread_kill(otherPthread, this->recoveryMgr->neutralizeSignal)) {
                // should never happen
                for (int i = 0; i < 20; ++i) COUTATOMICTID(
                        "######################################################" << std::endl);
                COUTATOMICTID("error " << error << " when trying to pthread_kill(pthread_tFor(" << otherTid << "), "
                                       << this->recoveryMgr->neutralizeSignal << ")" << std::endl);
                assert(isQuiescent(tid));
                const long newann = announcedEpoch[otherTid * PREFETCH_SIZE_WORDS].load(std::memory_order_relaxed);
                COUTATOMICTID("otherThread has newann=" << newann << " with quiescent bit " << QUIESCENT(newann)
                                                        << std::endl);
...
public:
    ...
    inline bool isQProtected(const int tid, T *const obj) {
        return announce[tid]->contains(
                obj); // this is inefficient, but should only happen when recovering from being neutralized...
    }

    inline bool qProtect(const int tid, T *const obj, CallbackType notRetiredCallback, CallbackArg callbackArg,
                         bool memoryBarrier = true) {
        TRACE COUTATOMICTID(
                "reclaimer_debraplus::protectObjectEvenIfQuiescent(tid=" << tid <</*", "<<*obj<<*/")" << std::endl);
        int __size;
        DEBUG __size = announce[tid]->size();
    ...
    inline void rotateEpochBags(const int tid) {
        ...
        if (freeable->getSizeInBlocks() >= scanThreshold) {
            ...
            TRACE COUTATOMICTID("retiring... we have " << freeable->computeSize()
                                                       << " things waiting to be retired in this epoch bag...");
            ...
            for (int otherTid = 0; otherTid < this->NUM_PROCESSES; ++otherTid) {
                ...
                for (int ixHP = 0; ixHP < sz; ++ixHP) {
                    ...
                    if (hp) {
                        int oldSize;
                        DEBUG2 oldSize = comparing[tid]->size();
                        comparing[tid]->insert((T *) hp);
                        DEBUG2 assert(comparing[tid]->size() <= oldSize +
                                                                1); // might not increase size if comparing[tid] already contains this item...
                    }
                }
            }
...
//        }
    ...
    reclaimer_debraplus(const int numProcesses, Pool *_pool, debugInfo *const _debug,
                        RecoveryMgr<void *> *const _recoveryMgr = NULL)
            : reclaimer_interface<T, Pool>(numProcesses, _pool, _debug, _recoveryMgr) {
        VERBOSE DEBUG COUTATOMIC("constructor reclaimer_debraplus helping=" << this->shouldHelp()
                                                                            << std::endl);// scanThreshold="<<scanThreshold<<std::endl;
        if (_recoveryMgr) COUTATOMIC(
                "SIGRTMIN=" << SIGRTMIN << " neutralizeSignal=" << this->recoveryMgr->neutralizeSignal
                            << std::endl);
