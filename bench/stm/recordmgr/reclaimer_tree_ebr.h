...
#   define EBRT_ROOT (1)
    ...
    public:
        epoch_tree(const int numThreads)
                : numThreadsPow2(roundUpPow2(numThreads)), numNodes(2 * numThreadsPow2),
...
//                        GSTATS_ADD(tid, num_prop_root, 1);
                        ...
                        if (parentVal != val)
                            return; // cannot propagate (value already propagated. we know this because val cannot be > parentVal, since we read val from root)
...
public:
    ...
    struct rebind {
        typedef reclaimer_tree_ebr<_Tp1, Pool> other;
    };
    template<typename _Tp1, typename _Tp2>
    struct rebind2 {
        typedef reclaimer_tree_ebr<_Tp1, _Tp2> other;
    };
    ...
    inline static bool protect(const int tid, T *const obj, CallbackType notRetiredCallback, CallbackArg callbackArg,
                               bool memoryBarrier = true) { return true; }
...
//        }
    ...
    reclaimer_tree_ebr(const int numProcesses, Pool *_pool, debugInfo *const _debug,
                       RecoveryMgr<void *> *const _recoveryMgr = NULL)
            : reclaimer_interface<T, Pool>(numProcesses, _pool, _debug, _recoveryMgr) {
        VERBOSE std::cout << "constructor reclaimer_tree_ebr helping=" << this->shouldHelp()
