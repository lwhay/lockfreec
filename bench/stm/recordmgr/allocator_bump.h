...
//        PAD; // not needed because of padding on end of previous array
    ...
    T *bump_memory_next(const int tid) {
        ...
        current[tid * PREFETCH_SIZE_WORDS] = (T *) (((char *) current[tid * PREFETCH_SIZE_WORDS]) +
                                                    (cachelines * BYTES_IN_CACHE_LINE));
        return result;
    }

    int bump_memory_bytes_remaining(const int tid) {
        return (((char *) mem[tid * PREFETCH_SIZE_WORDS]) + memBytes[2 * tid * PREFETCH_SIZE_WORDS]) -
               ((char *) current[tid * PREFETCH_SIZE_WORDS]);
    }

    bool bump_memory_full(const int tid) {
        return (((char *) current[tid * PREFETCH_SIZE_WORDS]) + cachelines * BYTES_IN_CACHE_LINE >
                ((char *) mem[tid * PREFETCH_SIZE_WORDS]) + memBytes[2 * tid * PREFETCH_SIZE_WORDS]);
    }
    ...
    void bump_memory_allocate(const int tid) {
        ...
        memBytes[2 * tid * PREFETCH_SIZE_WORDS] = 1 << 24;
        ...
        if (mod > 0) {
            ...
            current[tid * PREFETCH_SIZE_WORDS] = (T *) (((char *) mem[tid * PREFETCH_SIZE_WORDS]) +
                                                        BYTES_IN_CACHE_LINE - mod);
        } else {
            current[tid * PREFETCH_SIZE_WORDS] = mem[tid * PREFETCH_SIZE_WORDS];
        }
#endif
...
public:
    ...
    T *allocate(const int tid) {
        ...
        if (!mem[tid * PREFETCH_SIZE_WORDS] || bump_memory_full(tid)) {
            ...
            MEMORY_STATS {
                this->debug->addAllocated(tid,
                                          memBytes[2 * tid * PREFETCH_SIZE_WORDS] / cachelines / BYTES_IN_CACHE_LINE);
                VERBOSE DEBUG2 {
//                        if ((this->debug->getAllocated(tid) % 2000) == 0) {
//                            this->debugInterfaces->reclaim->debugPrintStatus(tid);
//                            debugPrintStatus(tid);
                        COUTATOMICTID("allocated " << (memBytes[2 * tid * PREFETCH_SIZE_WORDS] / cachelines /
                                                       BYTES_IN_CACHE_LINE)/*this->debug->getAllocated(tid)*/
                                                   << " records of size " << sizeof(T) << std::endl);
//                            COUTATOMIC(" ");
