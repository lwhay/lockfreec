...
//    PAD;
    ...
    int bump_memory_bytes_remaining(const int tid) {
        return (((char *) mem[tid]) + memBytes[tid * PREFETCH_SIZE_WORDS]) -
    ...
    bool bump_memory_full(const int tid) {
        return (((char *) current[tid * PREFETCH_SIZE_WORDS]) + cachelines * BYTES_IN_CACHE_LINE >
                ((char *) mem[tid]) + memBytes[tid * PREFETCH_SIZE_WORDS]);
    }
...
//        VERBOSE COUTATOMICTID(" finished touching each page."<<std::endl);
    ...
    ~allocator_once() {
        ...
        VERBOSE COUTATOMIC("destructor allocator_once allocated=" << allocated << " bytes, or "
                                                                  << (allocated / (cachelines * BYTES_IN_CACHE_LINE))
                                                                  << " objects of size " << sizeof(T) << " occupying "
                                                                  << cachelines << " cache lines" << std::endl);
