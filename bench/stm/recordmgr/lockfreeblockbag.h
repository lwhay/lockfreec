...
public:
    lockfreeblockbag() {
        VERBOSE DEBUG
        std::cout << "constructor lockfreeblockbag lockfree=" << head.is_lock_free() << std::endl;
    ...
    ~lockfreeblockbag() {
        VERBOSE DEBUG
        std::cout << "destructor lockfreeblockbag; ";
        ...
        VERBOSE DEBUG
        std::cout << "freed " << debugFreed << std::endl;
    }
    ...
    block<T> *getBlock() {
        while (true) {
            ...
            if (expHead.ptr != NULL) {
                if (head.compare_exchange_weak(expHead, tagged_ptr({expHead.ptr->next, expHead.tag + 1}))) {
    ...
    void addBlock(block<T> *b) {
        while (true) {
            tagged_ptr expHead = head.load(std::memory_order_relaxed);
            b->next = expHead.ptr;
            if (head.compare_exchange_weak(expHead, tagged_ptr({b, expHead.tag + 1}))) {
                return;
            }
        }
    }
