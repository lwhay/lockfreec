...
//                }
//                /******* end consistency check for concurrent iteration *******/
            ix = (curr ? curr->computeSize() - 1
                       : -1);                       // race condition: if reclamation happens AND curr is freed along with too many other blocks to fit in the blockpool, then this access might fault
//                /****** start consistency check for concurrent iteration ******/
//                assert(reclaimCountStart == bag->getReclaimCount());
//                if (reclaimCountStart != bag->getReclaimCount()) {
//                    // bag is being/has been reclaimed, so we cannot iterate
...
//            VERBOSE DEBUG std::cout<<" freed "<<debugFreed<<std::endl;
    ...
    void add(T *const obj) {
        DEBUG2 validate();
        int oldsize;
        DEBUG2 oldsize = computeSize();
        head->push(obj);
        if (head->isFull()) {
            int oldNumBlocks;
            DEBUG2 oldNumBlocks = computeSizeInBlocks();
...
//                MEMORY_STATS2 alloc->debug->addFromPool(tid, 1);
    ...
    block<T> *removeFullBlock() {
        ...
        DEBUG2
        if (sizeInBlocks != computeSizeInBlocks()) {
            std::cout << "sizeInBlocks=" << sizeInBlocks << " compute=" << computeSizeInBlocks() << std::endl;
        }
        DEBUG2 assert(sizeInBlocks == computeSizeInBlocks());
        DEBUG2 validate();
        return 0;
