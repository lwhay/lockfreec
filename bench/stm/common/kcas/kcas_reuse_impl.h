...
struct rdcssdesc_t {
    ...
    const static int size =
            sizeof(seqBits) + sizeof(addr1) + sizeof(old1) + sizeof(addr2) + sizeof(old2) + sizeof(new2);
...
void KCASLockFree<MAX_K>::rdcssHelp(rdcsstagptr_t tagptr, rdcssptr_t snapshot, bool helpingOther) {
    bool readSuccess;
    casword_t v = DESC_READ_FIELD(readSuccess, *snapshot->addr1, snapshot->old1, KCAS_SEQBITS_MASK_STATE,
...
bool KCASLockFree<MAX_K>::help(kcastagptr_t tagptr, kcasptr_t snapshot, bool helpingOther) {
    ...
    if (state == KCAS_STATE_UNDECIDED) {
        ...
        SEQBITS_CAS_FIELD(successBit, ptr->seqBits, snapshot->seqBits, KCAS_STATE_UNDECIDED, newstate,
...
template<int MAX_K>
casword_t KCASLockFree<MAX_K>::readPtr(casword_t volatile *addr) {
    casword_t r;
    do {
        r = rdcssRead(addr);
        if (isKcas(r)) {
...
template<int MAX_K>
casword_t KCASLockFree<MAX_K>::readVal(casword_t volatile *addr) {
    return ((casword_t) readPtr(addr)) >> KCAS_LEFTSHIFT;
}

template<int MAX_K>
void KCASLockFree<MAX_K>::writeInitPtr(casword_t volatile *addr, casword_t const newval) {
    *addr = newval;
}

template<int MAX_K>
void KCASLockFree<MAX_K>::writeInitVal(casword_t volatile *addr, casword_t const newval) {
    writeInitPtr(addr, newval << KCAS_LEFTSHIFT);
}

template<int MAX_K>
void KCASLockFree<MAX_K>::start() {
...
template<int MAX_K>
kcasptr_t KCASLockFree<MAX_K>::getDescriptor() {
...
template<int MAX_K>
template<typename T>
void KCASLockFree<MAX_K>::add(casword <T> *caswordptr, T oldVal, T newVal) {
