...
#include "plaf.h"
#include "bench/stm/descriptors/descriptors.h"
#include <cstring>
...
private:
    ...
    int help(const int tid, tagptr_t const tagptr, SCXRecord const *const snap, bool helpingOther) {
        ...
        for (int i = helpingOther; i < snap->c.numNodes; ++i) {
            bool successfulCAS = __sync_bool_compare_and_swap(&snap->c.nodes[i]->scxPtr,
                                                              (void *) snap->c.scxPtrsSeen[i], tagptr);
        ...
        MUTABLES1_WRITE_FIELD(ptr->c.mutables, snap->c.mutables, SCXRecord::STATE_COMMITTED, MUTABLES1_MASK_STATE,
                              MUTABLES1_OFFSET_STATE);
...
public:
    ...
    SCXProvider(const int _numThreads)
        ...
        for (int i = 0; i < numThreads; ++i) {
            DESC1_NEW(
                    i); // add a DESC1_NEW call at the start for each thread, since we only perform DESC1_NEW AFTER an scx
        }
    ...
    scx_handle_t
    llx(const int tid, NodeT const *const srcNode, NodeT *const dest = NULL, const int bytesToCopy = sizeof(NodeT)) {
