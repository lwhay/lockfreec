...
template<typename skey_t, typename sval_t, class RecMgr, class Compare>
void natarajan_ext_bst_lf<skey_t, sval_t, RecMgr, Compare>::retireDeletedNodes(thread_data_t<skey_t, sval_t> *data,
                                                                               node_t<skey_t, sval_t> *node,
                                                                               node_t<skey_t, sval_t> *targetNode,
                                                                               bool pointerFlagged) {
    ...
    if (is_flagged(node->child.AO_val1) || is_flagged(node->child.AO_val2)) {
        recmgr->retire(data->id, node);
        if (!is_free(node->child.AO_val1))
            retireDeletedNodes(data, (node_t<skey_t, sval_t> *) get_addr(node->child.AO_val1), targetNode,
                               is_flagged(node->child.AO_val1));
        if (!is_free(node->child.AO_val2))
            retireDeletedNodes(data, (node_t<skey_t, sval_t> *) get_addr(node->child.AO_val2), targetNode,
                               is_flagged(node->child.AO_val2));
    }
}
