...
#define nodeptr Node<K,V> * volatile
    ...
    public:
        ...
        bst_rwlock(const K _NO_KEY,
                   const V _NO_VALUE,
                   const int numProcesses)
                : NO_KEY(_NO_KEY), NO_VALUE(_NO_VALUE), recmgr(new RecManager(numProcesses)) {
            VERBOSE DEBUG COUTATOMIC("constructor bst_rwlock" << std::endl);
            cmp = Compare();
        ...
        ~bst_rwlock() {
            VERBOSE DEBUG COUTATOMIC("destructor bst_rwlock");
