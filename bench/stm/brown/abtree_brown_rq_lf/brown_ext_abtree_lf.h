...
//        PAD;
        ...
        abtree(const int numProcesses,
               ...
               int suspectedCrashSignal = SIGQUIT)
                : ALLOW_ONE_EXTRA_SLACK_PER_NODE(true), b(DEGREE), a(std::max(DEGREE / 4, 2)),
                  recordmgr(new RecManager(numProcesses, suspectedCrashSignal)), rqProvider(
                        new RQProvider<K, void *, Node<DEGREE, K>, abtree<DEGREE, K, Compare, RecManager>, RecManager, false, false>(
                                numProcesses, this, recordmgr)), NO_VALUE((void *) -1LL), NUM_PROCESSES(numProcesses) {
            cmp = Compare();
