...
#include "brown_bst_hohlock_impl.h"

#define RECORD_MANAGER_T record_manager<Reclaim, Alloc, Pool, bst_hohlock_ns::Node<K, V>>
#define DATA_STRUCTURE_T bst_hohlock_ns::bst_hohlock<K, V, std::less<K>, RECORD_MANAGER_T>
...
public:
    ds_adapter(const int NUM_THREADS,
            ...
            : NO_VALUE(VALUE_RESERVED), ds(new DATA_STRUCTURE_T(KEY_RESERVED, NO_VALUE, NUM_THREADS)) {}
    ...
    void printObjectSizes() {
        std::cout << "sizes: node="
                  << (sizeof(bst_hohlock_ns::Node<K, V>))
                  << std::endl;
    }
    ...
    class NodeHandler {
    public:
        typedef bst_hohlock_ns::Node<K,V> * NodePtrType;
        K minKey;
        K maxKey;
