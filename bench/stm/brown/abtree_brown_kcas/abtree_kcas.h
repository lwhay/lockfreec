...
private:
    ...
    long validateSubtree(Node<K, V, DEGREE> *node, std::unordered_set<K> &keys, ofstream &graph, ofstream &log,
                         bool &errorFound);
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline int
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::getChildIndex(NodeInternal<K, V, DEGREE> *node, const K &key) {
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline int
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::getKeyIndex(NodeExternal<K, V, DEGREE> *node, const K &key) {
    int nkeys = getKeyCount((Node<K, V, DEGREE> *) node);
    int retval = 0;
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
inline bool
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::validatePath(const int tid, const int &size, ObservedNode path[]) {
    assert(size > 0);
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
NodeInternal<K, V, DEGREE> *
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::createInternalNode(const int tid, bool weight, int size,
                                                                     K searchKey) {
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
NodeExternal<K, V, DEGREE> *
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::createExternalNode(const int tid, bool weight, int size,
                                                                     K searchKey) {
 ...
 */
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
int ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::searchTarget(const int tid, SearchInfo &info,
                                                                   Node<K, V, DEGREE> *target, const K &key) {
    auto &path = paths[tid].path;
    Node<K, V, DEGREE> *node;
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
int ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::insert(const int tid, SearchInfo &info, const K &key,
                                                             const V &value) {
...
V ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::tryErase(const int tid, const K &key) {
    ...
    while (true) {
        ...
        int res;
        while ((res = (search(tid, info, key))) == RetCode::RETRY || IS_MARKED(info.oParent.oVNumMark) ||
               IS_MARKED(info.oNode.oVNumMark)) {
            /* keep trying until we get a result */
        }
...
int ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::erase(const int tid, SearchInfo &info, const K &key) {
    ...
    caswordarraycopy(node->keys, info.keyIndex + 1, replacementNode->keys, info.keyIndex,
                     getKeyCount((Node<K, V, DEGREE> *) node) - (info.keyIndex + 1));
    caswordarraycopy(node->ptrs, 0, replacementNode->ptrs, 0, info.keyIndex);
    caswordarraycopy(node->ptrs, info.keyIndex + 1, replacementNode->ptrs, info.keyIndex,
                     node->size - (info.keyIndex + 1));
...
int ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::fixWeightViolation(const int tid, Node<K, V, DEGREE> *viol) {
    ...
    while (true) {
        ...
        while (searchTarget(tid, info, viol, viol->searchKey) == RetCode::RETRY || IS_MARKED(info.oNode.oVNumMark) ||
               IS_MARKED(info.oParent.oVNumMark) || IS_MARKED(info.oGParent.oVNumMark)) {
            /* keep trying until we get a result */
        }
        ...
        if (size <= b) {
            ...
            caswordarraycopy(parent->ptrs, info.nodeIndex + 1, absorber->ptrs, info.nodeIndex + node->size,
                             parent->size - (info.nodeIndex + 1));
            ...
            arraycopy(parent->keys, info.nodeIndex, absorber->keys,
                      info.nodeIndex + getKeyCount((Node<K, V, DEGREE> *) node),
                      getKeyCount((Node<K, V, DEGREE> *) parent) - info.nodeIndex);
            //hate this
            ...
            arraycopy(parent->ptrs, info.nodeIndex + 1, ptrs, info.nodeIndex + node->size,
                      parent->size - (info.nodeIndex + 1));
            arraycopy(parent->keys, 0, keys, 0, info.nodeIndex);
            arraycopy(node->keys, 0, keys, info.nodeIndex, getKeyCount((Node<K, V, DEGREE> *) node));
            arraycopy(parent->keys, info.nodeIndex, keys, info.nodeIndex + getKeyCount((Node<K, V, DEGREE> *) node),
                      getKeyCount((Node<K, V, DEGREE> *) parent) - info.nodeIndex);
    ...
    while (true) {
        ...
        if (size < 2 * a) {
            ...
            if (gParent == entry && parent->size == 2) {
                ...
                kcas::add(
                        &gParent->ptrs[info.parentIndex], (Node<K, V, DEGREE> *) parent,
                        (Node<K, V, DEGREE> *) newParent,
                        &gParent->vNumMark, info.oGParent.oVNumMark, info.oGParent.oVNumMark + 2,
                        &parent->vNumMark, info.oParent.oVNumMark, info.oParent.oVNumMark + 3,
                        &node->vNumMark, info.oNode.oVNumMark, info.oNode.oVNumMark + 3,
                        &sibling->vNumMark, oSibling.oVNumMark, oSibling.oVNumMark + 3
...
template<class RecordManager, typename K, typename V, int DEGREE, class Compare>
long
ABTreeKCAS<RecordManager, K, V, DEGREE, Compare>::validateSubtree(Node<K, V, DEGREE> *node, std::unordered_set<K> &keys,
