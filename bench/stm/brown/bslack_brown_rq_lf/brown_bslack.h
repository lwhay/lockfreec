...
#define BSLACK_ENABLE_DESTRUCTOR
    ...
    struct Node {
        ...
        void printTreeFile(std::ostream &os, std::set<bslack_ns::Node<DEGREE, K> *> *seen) {
            ...
            os << "(" << ((__state & bslack_ns::SCXRecord<DEGREE, K>::STATE_COMMITTED) ? "" : (__state &
                                                                                               bslack_ns::SCXRecord<DEGREE, K>::STATE_ABORTED)
                                                                                              ? "A" : "I")
               << (marked ? "m" : "")
               //               <<getKeyCount()
               << (weight ? "w1" : "w0")
...
#define arraycopy_ptrs(src, srcStart, dest, destStart, len) \
    ...
    private:
        std::string tagptrToString(uintptr_t tagptr) {
            ...
            if (tagptr) {
                if ((void *) tagptr == DUMMY) {
                    ...
                    ss << " ";
                    ss << "allFrozen="
                       << MUTABLES1_UNPACK_FIELD(mutables, MUTABLES1_MASK_ALLFROZEN, MUTABLES1_OFFSET_ALLFROZEN);
                    ss << " ";
        ...
        bool llx(const int tid, Node<DEGREE, K> *r, Node<DEGREE, K> **snapshot, const int i, SCXRecord<DEGREE, K> **ops,
                 Node<DEGREE, K> **nodes);
...
//        PAD;
        ...
        bslack(const int numProcesses,
               ...
               int suspectedCrashSignal = SIGQUIT)
                : ALLOW_ONE_EXTRA_SLACK_PER_NODE(true), b(DEGREE)
#ifdef USE_SIMPLIFIED_ABTREE_REBALANCING
                , a(DEGREE/2 - 2)
#endif
                , recordmgr(new RecManager(numProcesses, suspectedCrashSignal)), rqProvider(
                        new RQProvider<K, void *, Node<DEGREE, K>, bslack<DEGREE, K, Compare, RecManager>, RecManager, false, false>(
                                numProcesses, this, recordmgr)), SEQUENTIAL_STAT_TRACKING(false),
                  NO_VALUE((void *) -1LL), NUM_PROCESSES(numProcesses)
#ifdef USE_DEBUGCOUNTERS
        ...
        bool validate(const long long keysum, const bool checkkeysum) {
            if (checkkeysum) {
                ...
                if (treekeysum != keysum) {
                    std::cerr << "ERROR: tree keysum " << treekeysum << " did not match thread keysum " << keysum
                              << std::endl;
                    return false;
                }
            }
