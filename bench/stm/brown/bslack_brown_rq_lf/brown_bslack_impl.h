...
bslack_ns::SCXRecord<DEGREE, K> *
bslack_ns::bslack<DEGREE, K, Compare, RecManager>::createSCXRecord(const int tid, wrapper_info<DEGREE, K> *info) {
...
template<int DEGREE, typename K, class Compare, class RecManager>
int bslack_ns::bslack<DEGREE, K, Compare, RecManager>::rangeQuery(const int tid, const K &lo, const K &hi,
    ...
    while (!stack.isEmpty()) {
        ...
        if (node->isLeaf()) {
            ...
            int r = nkeys;
            while (r > 0 && cmp(hi, (const K &) node->keys[r - 1]))
...
template<int DEGREE, typename K, class Compare, class RecManager>
void *bslack_ns::bslack<DEGREE, K, Compare, RecManager>::doInsert(const int tid, const K &key, void *const value,
                                                                  const bool replace) {
    ...
    while (true) {
        ...
        if (keyIndex < l->getKeyCount() && l->keys[keyIndex] == key) {
            ...
            arraycopy(l->ptrs, 0, n->ptrs, 0,
                      l->getABDegree());    // although we are copying l->ptrs, since l is a leaf, l->ptrs CANNOT contain modified by rqProvider->linearize_update_at_..., so we do not use arraycopy_ptrs.
            ...
            if (l->getKeyCount() < b) {
                ...
                n->keys[keyIndex] = key;
                arraycopy(l->ptrs, 0, n->ptrs, 0,
                          keyIndex); // although we are copying the ptrs array, since the source node is a leaf, ptrs CANNOT contain modified by rqProvider->linearize_update_at_..., so we do not use arraycopy_ptrs.
                ...
                if (scx(tid, info)) {
                    TRACE COUTATOMICTID(
                            "insert std::pair (" << key << ", " << value << "): SCX succeeded" << std::endl);
#ifndef REBALANCING_NONE
#ifdef USE_SIMPLIFIED_ABTREE_REBALANCING
                    fixDegreeOrSlackViolation(tid, n);
#endif
                ...
                arraycopy(ptrs, 0, left->ptrs, 0,
                          size1); // although we are copying the ptrs array, since the node is a leaf, ptrs CANNOT contain modified by rqProvider->linearize_update_at_..., so we do not use arraycopy_ptrs.
                left->leaf = true;
                left->marked = false;
                left->scxPtr = DUMMY;
                ...
                arraycopy(ptrs, size1, right->ptrs, 0,
                          size2); // although we are copying the ptrs array, since the node is a leaf, ptrs CANNOT contain modified by rqProvider->linearize_update_at_..., so we do not use arraycopy_ptrs.
                right->leaf = true;
                right->marked = false;
                right->scxPtr = DUMMY;
...
const std::pair<void *, bool> bslack_ns::bslack<DEGREE, K, Compare, RecManager>::erase(const int tid, const K &key) {
    ...
    while (true) {
        ...
        if (keyIndex == l->getKeyCount() || l->keys[keyIndex] != key) {
            ...
            arraycopy(l->ptrs, 0, n->ptrs, 0,
                      keyIndex); // although we are copying the ptrs array, since the node is a leaf, ptrs CANNOT contain modified by rqProvider->linearize_update_at_..., so we do not use arraycopy_ptrs.
...
bool bslack_ns::bslack<DEGREE, K, Compare, RecManager>::fixWeightViolation(const int tid, Node<DEGREE, K> *viol) {
    ...
    while (true) {
        ...
        if (!llx(tid, gp, NULL, 0, info->scxPtrs, info->nodes) ||
            rqProvider->read_addr(tid, &gp->ptrs[ixToP]) != p)
            continue;    // retry the search
...
//            info->insertedNodes[0] = info->deletedNodes[0] = NULL;
            ...
            if (scx(tid, info)) {
                ...
                //        [maybe create slack at p]
                fixWeightViolation(tid, n);
                fixDegreeOrSlackViolation(tid,
                                          n);       // corresponds to node n using the terminology of the preceding comment
                fixDegreeOrSlackViolation(tid,
                                          left);    // corresponds to node n.p1 using the terminology of the preceding comment
                fixDegreeOrSlackViolation(tid,
                                          right);   // corresponds to node n.p2 using the terminology of the preceding comment
                fixDegreeOrSlackViolation(tid,
                                          gp);      // corresponds to node p using the terminology of the preceding comment
#endif
                return true;
            }
...
template<int DEGREE, typename K, class Compare, class RecManager>
bool
bslack_ns::bslack<DEGREE, K, Compare, RecManager>::fixDegreeOrSlackViolation(const int tid, Node<DEGREE, K> *viol) {
#ifdef REBALANCING_WEIGHT_ONLY
    return false;
#else
    /**
    ...
    while (true) {
        ...
        for (int i = 0; i < nodesWithFloor; ++i) {
            Node<DEGREE, K> *child = allocateNode(tid);
            arraycopy(keys, degreePerNodeCeil * nodesWithCeil + degreePerNodeFloor * i, child->keys, 0,
                      degreePerNodeFloor - !pChildrenAreLeaves);
            if (pChildrenAreLeaves) {
                arraycopy(ptrs, degreePerNodeCeil * nodesWithCeil + degreePerNodeFloor * i, child->ptrs, 0,
                          degreePerNodeFloor);
            } else {
                arraycopy_ptrs(ptrs, degreePerNodeCeil * nodesWithCeil + degreePerNodeFloor * i, child->ptrs, 0,
                               degreePerNodeFloor);
            }
            child->leaf = pChildrenAreLeaves;
            child->marked = false;
            ...
            child->searchKey = (numberOfNewChildren == 1) ? pChildren[0]->searchKey : keys[
                    degreePerNodeCeil * nodesWithCeil + degreePerNodeFloor * i];
        ...
        if (gp == entry && numberOfNewChildren == 1) {
            ...
            info->numberOfNodesToFreeze = 1 + 1 + (pChildrenAreLeaves ? 0
                                                                      : p->getABDegree()); // gp + p (since leaves cannot change, there is no need to freeze the children of p)
            info->field = &gp->ptrs[ixToP];
...
//            info->insertedNodes[0] = info->deletedNodes[0] = NULL;
            ...
            if (pChildrenAreLeaves) {
                ...
                for (int i = 0; i < nodesWithFloor -
                                    1; ++i) { // this is nodesWithFloor - 1 because we want to go up to numberOfNewChildren - 1, not numberOfNewChildren.
                    n->keys[i + nodesWithCeil] = keys[degreePerNodeCeil * nodesWithCeil + degreePerNodeFloor * i +
                                                      degreePerNodeFloor - 1];
                }
            }
            n->leaf = false;
            n->marked = false;
...
template<int DEGREE, typename K, class Compare, class RecManager>
bool
bslack_ns::bslack<DEGREE, K, Compare, RecManager>::llx(const int tid, Node<DEGREE, K> *r, Node<DEGREE, K> **snapshot,
                                                       const int i, SCXRecord<DEGREE, K> **ops,
                                                       Node<DEGREE, K> **nodes) {
...
bslack_ns::bslack<DEGREE, K, Compare, RecManager>::llx(const int tid, Node<DEGREE, K> *r, Node<DEGREE, K> **snapshot) {
    ...
    bool succ;
    TRACE COUTATOMICTID("tagged ptr seq=" << UNPACK1_SEQ(tagptr) << " descriptor seq="
                                          << UNPACK1_SEQ(TAGPTR1_UNPACK_PTR(tagptr)->c.mutables) << std::endl);
    int state = DESC1_READ_FIELD(succ, TAGPTR1_UNPACK_PTR(tagptr)->c.mutables, tagptr, MUTABLES1_MASK_STATE,
                                 MUTABLES1_OFFSET_STATE);
    if (!succ) state = SCXRecord<DEGREE, K>::STATE_COMMITTED;
    TRACE {
        mutables_t debugmutables = TAGPTR1_UNPACK_PTR(tagptr)->c.mutables;
        COUTATOMICTID(
                "llx scxrecord succ=" << succ << " state=" << state << " mutables=" << debugmutables << " desc-seq="
                                      << UNPACK1_SEQ(debugmutables) << std::endl);
    }
    ...
    if (state == SCXRecord<DEGREE, K>::STATE_ABORTED ||
        ((state == SCXRecord<DEGREE, K>::STATE_COMMITTED) && !r->marked)) {
...
template<int DEGREE, typename K, class Compare, class RecManager>
int bslack_ns::bslack<DEGREE, K, Compare, RecManager>::help(const int tid, const tagptr_t tagptr,
                                                            SCXRecord<DEGREE, K> const *const snap,
                                                            const bool helpingOther) {
#ifdef NO_HELPING
    int IGNORED_RETURN_VALUE = -1;
    if (helpingOther) return IGNORED_RETURN_VALUE;
#endif
    TRACE COUTATOMICTID("help " << tagptrToString(tagptr) << " helpingOther=" << helpingOther << " numNodes="
                                << snap->c.numberOfNodes << " numToFreeze=" << snap->c.numberOfNodesToFreeze
                                << std::endl);
    ...
    for (int i = helpingOther; i < snap->c.numberOfNodesToFreeze; ++i) {
        if (snap->c.nodes[i]->isLeaf()) {
            TRACE COUTATOMICTID((helpingOther ? "    " : "") << "help " << "nodes[" << i << "]@" << "0x"
                                                             << ((uintptr_t) (snap->c.nodes[i])) << " is a leaf\n");
        ...
        TRACE if (successfulCAS) COUTATOMICTID(
                    (helpingOther ? "    " : "") << "help froze nodes[" << i << "]@0x" << ((uintptr_t) snap->c.nodes[i])
                                                 << " with tagptr="
                                                 << tagptrToString((tagptr_t) snap->c.nodes[i]->scxPtr) << std::endl);
        ...
        bool succ;
        bool allFrozen = DESC1_READ_FIELD(succ, ptr->c.mutables, tagptr, MUTABLES1_MASK_ALLFROZEN,
                                          MUTABLES1_OFFSET_ALLFROZEN);
        if (!succ) return SCXRecord<DEGREE, K>::STATE_ABORTED;

        if (allFrozen) {
            TRACE COUTATOMICTID(
                    (helpingOther ? "    " : "") << "help return state " << SCXRecord<DEGREE comma K>::STATE_COMMITTED
                                                 << " after failed freezing cas on nodes[" << i << "]" << std::endl);
            return SCXRecord<DEGREE, K>::STATE_COMMITTED;
        } else {
            const int newState = SCXRecord<DEGREE, K>::STATE_ABORTED;
            TRACE COUTATOMICTID((helpingOther ? "    " : "") << "help return state " << newState
                                                             << " after failed freezing cas on nodes[" << i << "]"
                                                             << std::endl);
            MUTABLES1_WRITE_FIELD(ptr->c.mutables, snap->c.mutables, newState, MUTABLES1_MASK_STATE,
                                  MUTABLES1_OFFSET_STATE);
            return newState;
        }
    }
...
//    __sync_bool_compare_and_swap(snap->c.field, snap->c.nodes[1], snap->c.newNode);
    TRACE COUTATOMICTID(
            (helpingOther ? "    " : "") << "help CAS'ed to newNode@0x" << ((uintptr_t) snap->c.newNode) << std::endl);

    MUTABLES1_WRITE_FIELD(ptr->c.mutables, snap->c.mutables, SCXRecord<DEGREE comma K>::STATE_COMMITTED,
                          MUTABLES1_MASK_STATE, MUTABLES1_OFFSET_STATE);

    TRACE COUTATOMICTID(
            (helpingOther ? "    " : "") << "help return COMMITTED after performing update cas" << std::endl);
