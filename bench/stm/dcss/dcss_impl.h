...
dcssresult_t dcssProvider::dcssHelp(const int tid, dcsstagptr_t tagptr, dcssptr_t snapshot, bool helpingOther) {
    ...
    casword_t retval;
    bool failedBit;
    MUTABLES_VAL_CAS_FIELD(failedBit, retval, ptr->mutables, snapshot->mutables, DCSS_STATE_UNDECIDED, state,
                           DCSS_MUTABLES_MASK_STATE, DCSS_MUTABLES_OFFSET_STATE);
    if (failedBit)
        return {DCSS_IGNORED_RETVAL,
    ...
    if ((retval == DCSS_STATE_UNDECIDED && state == DCSS_STATE_SUCCEEDED)     // if we changed the state to succeeded OR
        || retval ==
           DCSS_STATE_SUCCEEDED) {                                     // if someone else changed the state to succeeded
//        if (state == DCSS_STATE_FAILED) DELAY_UP_TO(1000);
        assert(helpingOther ||
               ((snapshot->mutables & DCSS_MUTABLES_MASK_STATE) >> DCSS_MUTABLES_OFFSET_STATE) == DCSS_STATE_SUCCEEDED);
        ...
        assert(helpingOther ||
               ((snapshot->mutables & DCSS_MUTABLES_MASK_STATE) >> DCSS_MUTABLES_OFFSET_STATE) == DCSS_STATE_FAILED);
...
dcssresult_t dcssProvider::dcssVal(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                   casword_t new2) {
    return dcssPtr(tid, addr1, old1, addr2, old2 << DCSS_LEFTSHIFT, new2 << DCSS_LEFTSHIFT);
}

dcssresult_t dcssProvider::dcssPtr(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                   casword_t new2) {
