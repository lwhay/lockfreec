...
public:
    ...
    const static int size =
            sizeof(mutables) + sizeof(addr1) + sizeof(old1) + sizeof(addr2) + sizeof(old2) + sizeof(new2) +
            sizeof(PAYLOAD_T) * (MAX_PAYLOAD_PTRS + 1) + sizeof(PAYLOAD_T) * (MAX_PAYLOAD_PTRS + 1);
...
public:
    ...
    void writePtr(casword_t volatile *addr,
                  casword_t val);        // use for addresses that might have been modified by DCSSP (ONLY GOOD FOR INITIALIZING, CANNOT DEAL WITH CONCURRENT DCSSP OPERATIONS.)
    void writeVal(casword_t volatile *addr,
                  casword_t val);        // use for addresses that might have been modified by DCSSP (ONLY GOOD FOR INITIALIZING, CANNOT DEAL WITH CONCURRENT DCSSP OPERATIONS.)
    casword_t
    readPtr(const int tid, casword_t volatile *addr);    // use for addresses that might have been modified by DCSSP
    casword_t
    readVal(const int tid, casword_t volatile *addr);    // use for addresses that might have been modified by DCSSP
    inline dcsspresult_t
    dcsspPtr(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2, casword_t new2,
             PAYLOAD_T *const payload1,
             PAYLOAD_T *const payload2); // use when addr2 is a pointer, or another type that does not use its least significant bit
    inline dcsspresult_t
    dcsspVal(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2, casword_t new2,
             PAYLOAD_T *const payload1,
             PAYLOAD_T *const payload2); // use when addr2 uses its least significant bit, but does not use its most significant but
