...
public:
    ...
    void writePtr(casword_t volatile *addr,
                  casword_t val);        // use for addresses that might have been modified by DCSS (ONLY GOOD FOR INITIALIZING, CANNOT DEAL WITH CONCURRENT DCSS OPERATIONS.)
    void writeVal(casword_t volatile *addr,
                  casword_t val);        // use for addresses that might have been modified by DCSS (ONLY GOOD FOR INITIALIZING, CANNOT DEAL WITH CONCURRENT DCSS OPERATIONS.)
    casword_t
    readPtr(const int tid, casword_t volatile *addr);    // use for addresses that might have been modified by DCSS
    casword_t
    readVal(const int tid, casword_t volatile *addr);    // use for addresses that might have been modified by DCSS
    inline dcssresult_t dcssPtr(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                casword_t new2); // use when addr2 is a pointer, or another type that does not use its least significant bit
    inline dcssresult_t dcssVal(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                casword_t new2); // use when addr2 uses its least significant bit, but does not use its most significant but
    void debugPrint();
