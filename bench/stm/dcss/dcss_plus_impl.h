...
dcsspresult_t
dcsspProvider<PAYLOAD_T>::dcsspHelp(const int tid, dcssptagptr_t tagptr, dcsspptr_t snapshot, bool helpingOther) {
    ...
    casword_t retval;
    bool failedBit;
    MUTABLES_VAL_CAS_FIELD(failedBit, retval, ptr->mutables, snapshot->mutables, DCSSP_STATE_UNDECIDED, state,
                           DCSSP_MUTABLES_MASK_STATE, DCSSP_MUTABLES_OFFSET_STATE);
    if (failedBit)
        return {DCSSP_IGNORED_RETVAL,
                0};                             // failed to access the descriptor: we must be helping another process complete its operation, so we will NOT use this return value!
    ...
    if ((retval == DCSSP_STATE_UNDECIDED &&
         state == DCSSP_STATE_SUCCEEDED)     // if we changed the state to succeeded OR
        || retval ==
           DCSSP_STATE_SUCCEEDED) {                                     // if someone else changed the state to succeeded
//        if (state == DCSSP_STATE_FAILED) DELAY_UP_TO(1000);
        assert(helpingOther || ((snapshot->mutables & DCSSP_MUTABLES_MASK_STATE) >> DCSSP_MUTABLES_OFFSET_STATE) ==
        ...
        assert(helpingOther ||
               ((snapshot->mutables & DCSSP_MUTABLES_MASK_STATE) >> DCSSP_MUTABLES_OFFSET_STATE) == DCSSP_STATE_FAILED);
...
template<typename PAYLOAD_T>
dcsspresult_t
dcsspProvider<PAYLOAD_T>::dcsspVal(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                   casword_t new2, PAYLOAD_T *const payload1, PAYLOAD_T *const payload2) {
    return dcsspPtr(tid, addr1, old1, addr2, old2 << DCSSP_LEFTSHIFT, new2 << DCSSP_LEFTSHIFT, payload1, payload2);
}

template<typename PAYLOAD_T>
dcsspresult_t
dcsspProvider<PAYLOAD_T>::dcsspPtr(const int tid, casword_t *addr1, casword_t old1, casword_t *addr2, casword_t old2,
                                   casword_t new2, PAYLOAD_T *const payload1, PAYLOAD_T *const payload2) {
