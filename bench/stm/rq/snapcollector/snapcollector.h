    ...
    void retire(const int tid, RecordManager *recmgr) {
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            ...
            while (curr != NULL) {
                if (curr != blocker)
                    recmgr->retire(tid,
                                   curr); // blocker can exist in many per-thread lists, but we only want to retire it once, below.
                curr = curr->next;
            }
        }
        // retire blocker
...
private:
    ...
    template<typename RecordManager>
    void
    AddReports(const int tid, std::vector<CompactReportItem *> *allReports, ReportItem *curr, RecordManager *recmgr) {
        curr = curr->next;
