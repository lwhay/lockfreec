...
#define __RQ_THREAD_DATA_SIZE 1024
        union {
            struct { // anonymous struct inside anonymous union means we don't need to type anything special to access these variables
                ...
                PAD; // prevent false sharing between htm debugging stats below and announcements
...
public:
    ...
    ~RQProvider() {
        std::cout << "writer commits : " << dosum(commitWriter) << std::endl;
        std::cout << "writer aborts  : " << dosum(abortWriter) << std::endl;
        std::cout << "reader commits : " << dosum(commitReader) << std::endl;
        std::cout << "reader aborts  : " << dosum(abortReader) << std::endl;
        std::cout << "fallback       : " << dosum(fallback) << std::endl;
...
private:
    ...
    inline int
    __traversal_try_add(const int tid, NodeType *const node, NodeType **const nodeSource, K *const outputKeys,
                        V *const outputValues, const K &lo, const K &hi, bool foundDuringTraversal) {
        ...
        if (node->itime >= threadData[tid].rq_lin_time)
        ...
        if (dtime != TIMESTAMP_NOT_SET) {
            if (dtime < threadData[tid].rq_lin_time)
                return 0;                  // node was deleted before the range query
            goto tryAddToRQ;
        }

        // fact: dtime was not set above
        if (logicalDeletion && !logicallyDeleted)
        ...
        if (cnt == 0)
            return 0;                                                 // node doesn't contain any keys that are in the set
    ...
    inline void
    traversal_try_add(const int tid, NodeType *const node, NodeType **const nodeSource, K *const rqResultKeys,
                      V *const rqResultValues, int *const startIndex, const K &lo, const K &hi,
                      bool foundDuringTraversal) {
...
//        }
//#endif
        int numNewKeys = __traversal_try_add(tid, node, nodeSource, rqResultKeys + (*startIndex),
                                             rqResultValues + (*startIndex), lo, hi, foundDuringTraversal);
        ...
        for (int otherTid = 0; otherTid < NUM_PROCESSES; ++otherTid)
            if (otherTid != tid) {
                ...
                for (int i = 0; i < sz; ++i) {
                    ...
                    assert(node);
                    traversal_try_add(tid, node, (NodeType **) &threadData[otherTid].announcements[i], rqResultKeys,
                                      rqResultValues, startIndex, lo, hi, false);
                }
            }
        SOFTWARE_BARRIER;
