...
public:
    MultiCounter(const int numThreads, const int sizeMultiple)
            : counters(new SingleCounter[std::max(2, sizeMultiple * numThreads) +
                                         1]) // allocate one extra entry (don't use first entry---to effectively add padding at the start of the array)
