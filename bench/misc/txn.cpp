...
void txn_man::init(thread_t *h_thd, workload *h_wl, uint64_t thd_id) {
    ...
    wr_cnt = 0;
    insert_cnt = 0;
    accesses = (Access **) _mm_malloc(sizeof(Access *) * MAX_ROW_PER_TXN, ALIGNMENT);
...
row_t *txn_man::get_row(row_t *row, access_t type) {
    ...
    if (accesses[row_cnt] == NULL) {
        Access *access = (Access *) _mm_malloc(sizeof(Access), ALIGNMENT);
        accesses[row_cnt] = access;
#if (CC_ALG == SILO || CC_ALG == TICTOC)
        access->data = (row_t *) _mm_malloc(sizeof(row_t), ALIGNMENT);
        access->data->init(MAX_TUPLE_SIZE);
        access->orig_data = (row_t *) _mm_malloc(sizeof(row_t), ALIGNMENT);
...
int txn_man::index_range_query(Index * index, idx_key_t low, idx_key_t high, idx_key_t * resultKeys, itemid_t ** resultValues, int part_id) {
    uint64_t starttime = get_sys_clock();
        int numResults = 0;
