...
void Row_tictoc::init(row_t * row)
{
    ...
    _ts_word = 0;
#else
    _latch = (pthread_mutex_t *) _mm_malloc(sizeof(pthread_mutex_t), ALIGNMENT);
...
RC Row_tictoc::access(txn_man * txn, TsType type, row_t * local_row)
{
#if ATOMIC_WORD
    uint64_t v = 0;
    uint64_t v2 = 1;
...
void Row_tictoc::write_data(row_t * data, ts_t wts)
{
#if ATOMIC_WORD
      uint64_t v = _ts_word;
#if TICTOC_MV
...
bool Row_tictoc::renew_lease(ts_t wts, ts_t rts)
{
#if !ATOMIC_WORD
    if (_wts != wts) {
#if TICTOC_MV
...
bool Row_tictoc::try_renew(ts_t wts, ts_t rts, ts_t &new_rts, uint64_t thd_id)
{
#if ATOMIC_WORD
    uint64_t v = _ts_word;
...
void Row_tictoc::get_ts_word(bool &lock, uint64_t &rts, uint64_t &wts)
{
    assert(ATOMIC_WORD);
    uint64_t v = _ts_word;
    lock = ((v & LOCK_BIT) != 0);
